...
try {
    ...
    Write-Output "Published transiently: $output"
    if (-not $KeepPublishedPdf) {
        Remove-Item -LiteralPath $output -Force
        Write-Output 'Removed transient published PDF; use -KeepPublishedPdf only for a managed release.'
    }
}
finally {
    Remove-Item -LiteralPath $buildDirectory -Recurse -Force -ErrorAction SilentlyContinue
