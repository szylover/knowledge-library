...
\begin{itemize}
  ...
  \item \textbf{§\ref{sec:modular-curve-complex-structure}}
    把模曲线 $Y(\Gamma)$ 做成黎曼面：普通点与椭圆点各有不同的局部坐标。
  \item \textbf{§\ref{sec:compactification}}
    在尖点处添加局部坐标，得到紧黎曼面 $X(\Gamma)$。
  \item \textbf{§\ref{sec:riemann-hurwitz}}
    用 Riemann-Hurwitz 公式计算 $X(\Gamma)$ 的亏格。
  \item \textbf{§\ref{sec:differentials-and-modforms}}
    揭示模形式与全纯微分形式的对应关系。
  \item \textbf{§\ref{sec:ch02-exercises}}
    用习题把复结构、紧化、亏格与微分形式串起来。
\end{itemize}
...
\input{figures/ch02-riemann-surface-charts}
...
\input{chapters/ch04-riemann-content/s2-complex-structure}
\input{chapters/ch04-riemann-content/s3-compactification}
\input{chapters/ch04-riemann-content/s4-riemann-hurwitz}
\input{chapters/ch04-riemann-content/s5-differentials}
\input{chapters/ch04-riemann-content/s6-exercises}
