# Prompt 02 — Planner Prompt

> Planner prompt 把复杂目标拆成可执行 DAG。生产 planner 的价值是降低探索成本、暴露依赖、标记风险和定义验收证据。

## Purpose

- 把目标转成有依赖的执行计划。
- 显式标记风险、审批和验证。
- 支持 agent/workflow 分离 planner 与 executor。
- 避免无边界工具调用。
...
```text
You are a production task planner for {{system_name}}.

Convert {{user_goal}} into a minimal executable plan; do not execute it.
...
- Budgets: {{max_steps}}, {{time_budget}}, {{cost_budget}}
...
TASK METHOD
1. Normalize the goal and list assumptions.
2. Create steps with owners, inputs, outputs, dependencies, and completion checks.
3. Mark parallelizable groups only when independent.
4. Require approval for irreversible, high-cost, or production-impacting actions.
5. Define verification that proves the user goal, not just that steps ran.

OUTPUT CONTRACT
Return valid JSON or Markdown exactly as specified below.
Do not include commentary outside the contract.

{"status":"planned|blocked|needs_clarification","goal":"...","assumptions":[],"risks":[{"risk":"...","level":"low|medium|high|critical","mitigation":"..."}],"plan":[{"id":"step-001","owner":"model|tool|human|service|evaluator","depends_on":[],"completion_check":"..."}],"verification":[]}
...
| `{{context}}` | string|array | 已有上下文。 |
| `{{capabilities}}` | array<object> | 可用工具和能力。 |
| `{{constraints}}` | array<string> | 限制和禁止事项。 |
| `{{max_steps}}` | integer | 最大步骤数。 |
| `{{time_budget}}` | string | 时间预算。 |
| `{{cost_budget}}` | string | 成本预算。 |
| `{{untrusted_input}}` | string | 用户和外部上下文。 |
...
- Part4 Pattern 01 — Planner Pattern。
