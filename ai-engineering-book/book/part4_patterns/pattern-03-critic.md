# Pattern 03 — Critic Pattern

> 使用独立 critic 评估 actor 输出，把生成与判定解耦，形成质量门禁。

## Why

本章交叉引用：Part2 Ch15 Evaluation；Part2 Ch16 Guardrails；Part2 Ch10 RAG。

生产 AI 系统的主要风险不在单次回答，而在隐式决策不可观察。

Critic Pattern的目标是把模型行为拆成可校验、可记录、可回放的工程对象。
...
如果没有评测集和线上指标，Critic Pattern只会增加复杂度。

如果没有安全降级路径，Critic Pattern会把随机错误放大成系统事故。

Critic Pattern 的核心是让 `candidate` 和 `verdict` 可被验证，而不是让模型自由发挥。
...
- 需要审计模型为什么做出某个选择。
- 需要对供应商模型漂移做回归测试。
- 当 `candidate` 的选择会影响成本、质量或权限。
- 当 `verdict` 必须携带 evidence、trace 或 policy version。
...
- 没有观测、回放和评测能力。
- 用规则或普通代码更简单可靠时。
- 无法为 `candidate` 写出稳定 schema。
- 无法验证 `verdict` 是否满足业务语义。
...
flowchart TD
 U[Request]-->A[Actor]
 A-->O[Candidate]
 O-->D[Deterministic Checks]
 D-->C[Independent Critic]
 C-->V{Verdict}
 V-->|pass|R[Return]
 V-->|revise|A
 V-->|reject|S[Safe Failure]
...
### Critic-specific operating rules

- Critic 必须独立于 actor：不同 prompt、不同上下文窗口，必要时不同模型或确定性工具。
- Actor 的目标是完成任务；critic 的目标是发现问题，两个目标不要混在同一个系统提示里。
- Critic verdict 必须结构化：score、confidence、action、issues、severity、policy_refs。
- revise 与 reject 要分开；可修复问题给 actor，不可修复问题进入 safe failure。
- 低 confidence 的高风险 verdict 应 escalate，而不是强行 reject。
- Deterministic checks 应放在 LLM critic 前，例如 schema、权限、引用 id、单元测试。
- Critic 阈值必须用标注集校准；没有 false positive/negative 估计就不要上线强门禁。
- 防 reward hacking：actor 不应长期看到完整 critic 隐藏 rubric。
- Online critic 负责快速 gate；offline critic 负责深度抽检、阈值调优和漂移发现。
- 对 RAG，critic 应逐 claim 检查 evidence support，而不是只评整体流畅度。
- 对代码，critic 应优先运行测试、类型检查、静态分析，再用 LLM 看边界条件。
- 对安全，critic 输出必须能映射到 policy id，方便审计和申诉。

| Verdict | 语义 | 典型动作 |
|---|---|---|
| pass | 满足质量线 | 返回 |
| revise | 可修复缺陷 | actor 修订 |
| reject | 不可安全回答 | safe failure |
| escalate | 高风险或低置信 | 人工审核 |
| monitor | 线上可返回但需抽检 | offline queue |

## Pseudo Code

```text
function run_critic(request, context):
    features = extract_features(request, context)
    candidate = model_or_policy_decide(features, schema, budget)
    validation = validate_candidate(candidate, schema, policy)
    if not validation.ok:
        repaired = repair_or_safe_fail(validation)
        if repaired.failed: return repaired
        candidate = repaired.value
    for attempt in range(max_attempts):
        verdict = execute_or_evaluate(candidate, request, context)
        verdict = verify_verdict(verdict, acceptance_criteria)
        if verdict.pass: return synthesize(request, verdict)
        if verdict.terminal: return safe_failure(verdict)
        if budget.exhausted(): return partial_or_safe_failure(verdict)
        candidate = revise_decision(candidate, verdict, context)
...
class RuntimePolicy(BaseModel):
    ...
    policy_version: str = "critic-2026-07"
...
class CriticRuntime:
    ...
    async def decide(self, request: str, trace: CriticTrace) -> CriticDecision:
        if self.estimate_tokens(request) > self.policy.max_input_tokens:
            return CriticDecision(action=Action.ESCALATE, target="long_context_path", confidence=1.0, reason_codes=["token_budget_exceeded"])
        response = await self.client.responses.parse(
            model="gpt-4.1-mini",
            temperature=0.0,
            text_format=CriticDecision,
            input=[
                {"role": "system", "content": "Make a production critic decision. Return only structured JSON. Prefer safe failure over guessing."},
                {"role": "user", "content": f"Request: {request}\nTrace: {trace.model_dump()}"},
            ],
        )
        return response.output_parsed

    def validate_decision(self, decision: CriticDecision) -> None:
    ...
    async def execute(self, request: str, decision: CriticDecision) -> CriticResult:
        started = time.time()
        try:
            if decision.action in {Action.REJECT, Action.ESCALATE}:
                return CriticResult(ok=False, error=decision.action.value, latency_ms=int((time.time() - started) * 1000))
            ...
            return CriticResult(ok=True, content=response.output_text, evidence=decision.reason_codes, latency_ms=int((time.time() - started) * 1000))
        except Exception as exc:
            return CriticResult(ok=False, error=str(exc), latency_ms=int((time.time() - started) * 1000))

    def verify(self, decision: CriticDecision, result: CriticResult) -> Action:
        if decision.action in {Action.REJECT, Action.ESCALATE}:
    ...
    async def run(self, request: str) -> dict[str, Any]:
        trace = CriticTrace(request_id=str(uuid.uuid4()), policy_version=self.policy.policy_version)
        for _ in range(self.policy.max_attempts + 1):
            ...
            try:
                ...
                decision = CriticDecision(action=Action.REVISE, target="repair", confidence=0.0, reason_codes=[str(exc)])
...
- Critic Pattern 的价值是把概率输出纳入工程治理。
- 不要把模型当权限边界。
