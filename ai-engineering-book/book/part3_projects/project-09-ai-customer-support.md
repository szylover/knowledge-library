# Project 09 — AI Customer Support

> 把客服机器人从 FAQ wrapper 升级为可审计、可升级、可调用业务系统的 support operating layer；目标不是替代所有人工，而是在正确置信度下 deflect 简单问题、加速复杂工单、降低误操作。

---

## Business Goal

多语言企业客服系统，覆盖知识库问答、历史工单检索、订单/退款工具调用、人工升级和客服辅助摘要。

本章默认读者已经理解后端系统、分布式事务、缓存、队列和基本 LLM 概念；重点放在生产约束、失败域、质量闭环和成本治理。

交叉参考：Part1 Ch01 API 设计、Part1 Ch04 事件驱动、Part2 Ch07 RAG、Part2 Ch11 Memory、Part2 Ch15 Evaluation、Part2 Ch19 Safety。

| 维度 | 生产目标 | 不做什么 |
|---|---|---|
| 用户价值 | deflection rate、first contact resolution、CSAT、escalation precision、cost per resolved ticket。 | 不把 demo success 当作上线指标 |
| 平台价值 | 可审计、可回放、可限额 | 不把 prompt 当作唯一业务逻辑 |
| 工程价值 | 故障可恢复，质量可度量 | 不依赖人工看日志猜测问题 |
| 商业价值 | 单次任务毛利可计算 | 不让模型、图片或 token 成本黑盒增长 |

关键风险：错误退款、承诺不存在的政策、泄露订单信息、跨语言误解和 prompt injection 引导工具误用。
...
### User stories
- 作为终端用户，我可以提交 support conversations and tickets 请求并实时看到阶段状态，而不是等待一个黑盒 HTTP 超时。
...
flowchart LR
    ...
    W --> V[(Qdrant\nsupport_kb_{tenant})]
    W --> OBJ[Postgres ticket store + Redis hot session]
    W --> POL[Policy Engine\nGuardrails · HITL · DLP]
...
- 多轮上下文不是无限拼接；会话摘要、当前问题、订单事实和检索证据分层进入上下文。
...
| LLM | OpenAI GPT-4.1/4o-mini + Anthropic Claude Sonnet，按意图、语言和风险路由 | 质量、延迟和成本按任务路由 |
...
```text
SYSTEM:
You are the orchestration component for Project 09 AI Customer Support.
...
TOOLS: order_lookup, refund_quote, refund_create_draft, crm_note, escalation_create
```
...
KB、政策文档、产品手册、历史已解决 ticket、宏模板和多语言同义词表。
...
安全边界包括：HITL threshold、PII redaction、tool allowlist、refund amount policy、tenant language policy。AI 系统的特殊点是外部内容可能试图改变模型行为，因此 data 和 instruction 必须强隔离。
...
- AI Customer Support 的工程重点是边界治理，不是把 LLM 接到 UI。
