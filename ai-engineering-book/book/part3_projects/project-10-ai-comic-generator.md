# Project 10 — AI Comic Generator

> 漫画生成不是一次 image prompt；它是 story planning、角色一致性、分镜约束、异步渲染、内容安全、版权边界和成本控制共同组成的 multimodal production pipeline。

---

## Business Goal

为创作者和营销团队生成可编辑的短篇漫画：故事大纲、panel script、角色设定、图像生成、排版导出和 CDN 分发。

本章默认读者已经理解后端系统、分布式事务、缓存、队列和基本 LLM 概念；重点放在生产约束、失败域、质量闭环和成本治理。

交叉参考：Part1 Ch01 长任务 API、Part1 Ch04 队列与幂等、Part2 Ch01 LLM、Part2 Ch08 Multimodal、Part2 Ch15 Evaluation、Part2 Ch19 Safety。

| 维度 | 生产目标 | 不做什么 |
|---|---|---|
| 用户价值 | job completion rate、usable panel ratio、character consistency score、moderation precision、cost per comic。 | 不把 demo success 当作上线指标 |
| 平台价值 | 可审计、可回放、可限额 | 不把 prompt 当作唯一业务逻辑 |
| 工程价值 | 故障可恢复，质量可度量 | 不依赖人工看日志猜测问题 |
| 商业价值 | 单次任务毛利可计算 | 不让模型、图片或 token 成本黑盒增长 |

关键风险：角色漂移、图像模型成本爆炸、生成违规内容、版权风格侵权、长任务失败后无法恢复。
...
### User stories
- 作为终端用户，我可以提交 comic generation jobs and render assets 请求并实时看到阶段状态，而不是等待一个黑盒 HTTP 超时。
...
flowchart LR
    ...
    R --> W[comic-render-worker\nLangGraph Workers]
    W --> MG[Model Gateway\nOpenAI · Anthropic · Fallback]
    W --> V[(Qdrant\ncomic_style_{tenant})]
    W --> OBJ[Postgres job store + S3-compatible object storage + CDN]
    W --> POL[Policy Engine\nGuardrails · HITL · DLP]
...
- 角色一致性必须作为数据资产管理：reference image、seed、character embedding、negative prompt 和每次 render metadata 都要落库。
...
| LLM | LLM for story planning，image model for panels，CLIP/embedding verifier for consistency | 质量、延迟和成本按任务路由 |
...
```text
SYSTEM:
You are the orchestration component for Project 10 AI Comic Generator.
...
TOOLS: story_llm, panel_prompt_builder, image_generate, image_upscale, moderation_check, asset_publish
```
...
品牌素材库、角色设定、历史分镜模板、prompt style guide；不用于事实问答，而用于风格和约束检索。
...
services:
  ...
  worker:
    ...
    environment:
      QUEUE_NAME: comic-render-worker
      MAX_CONCURRENCY: "8"
  redis:
    image: redis:7
  postgres:
...
安全边界包括：内容 moderation、copyright style denylist、seed/reference governance、brand approval gate。AI 系统的特殊点是外部内容可能试图改变模型行为，因此 data 和 instruction 必须强隔离。
...
- AI Comic Generator 的工程重点是边界治理，不是把 LLM 接到 UI。
