# Project 05 — AI Browser Agent

> 把浏览器从“可被模型点击的 UI”升级为受治理的执行环境：每一次观察、计划、动作、回滚都必须可审计、可限额、可暂停。

## Business Goal

本项目服务的不是 demo 场景，而是企业里的浏览器自动化工作流。目标用户是运营、财务、供应链、内部工具重度用户；他们的痛点不是不会点按钮或不会写总结，而是流程分散、证据不足、成本不可控、权限边界模糊。

业务目标可以压缩成一句话：在没有稳定 API 的 SaaS/后台页面中完成跨页面任务，并且把每一次模型判断、工具调用、人工批准都变成可追踪的系统事件。
...
关键风险：页面内容携带 indirect prompt injection，诱导 agent 泄漏数据或执行危险动作。这决定了架构必须默认不信任外部内容，默认限制动作空间，默认把高风险行为交给 Human-in-the-loop。

- 参考：Part 1 Chapter 01 API 设计：长任务建模为 task 资源。
- 参考：Part 2 Chapter 01 LLM 基础：长上下文与 token 成本。
- 参考：Part 2 安全章节：prompt injection 与 tool guardrail。
...
- 作为业务用户，我可以提交一个自然语言目标，系统异步执行浏览器自动化任务，并实时看到进度。
...
ai-project/
  ...
  app/db/models.py
  app/workers/browser_worker.py
  tests/e2e/test_checkout_policy.py
```
...
```text
SYSTEM:
You are the planning component for AI Browser Agent.
...
工作流核心是 `observe → plan → act → verify → checkpoint`。不要把它实现成 while True + prompt；生产系统需要显式节点、可持久化状态、可恢复错误和明确终止条件。

```mermaid
flowchart TD
    N0[Start Task]
    N0[Start Task] --> N1[Load Site Policy]
    N1[Load Site Policy] --> N2[Observe DOM/A11y]
    N2[Observe DOM/A11y] --> N3[Plan Next Action]
    N3[Plan Next Action] --> N4[Guardrail Check]
    N4[Guardrail Check] --> N5[Execute Playwright Action]
    N5[Execute Playwright Action] --> N6[Verify State]
    N6[Verify State] --> N7[Need HITL?]
    N7[Need HITL?] --> N8[Persist Trajectory]
    N8[Persist Trajectory] --> N9[Done]
...
async def execute_action(page: Page, action: BrowserAction) -> ActionResult:
    ...
    try:
        if action.type == ActionType.CLICK:
            ...
            await page.locator(action.selector).fill(action.value or "", timeout=5_000)
        elif action.type == ActionType.SELECT:
            await page.locator(action.selector).select_option(action.value, timeout=5_000)
        else:
            await page.wait_for_load_state("networkidle", timeout=8_000)
        return ActionResult(ok=True, observation_id=await capture_observation(page))
    except PlaywrightTimeoutError as exc:
        return ActionResult(ok=False, observation_id=await capture_observation(page), error=str(exc))
```
...
本项目的 RAG 不是“相似度搜索补充上下文”，而是为 agent 提供可引用、可过滤、可过期的业务记忆：任务手册、站点 SOP、过往轨迹、元素定位经验、错误修复 playbook。
...
| 实体 | 用途 |
| --- | --- |
| browser_sessions | 领域状态或执行证据，必须可审计 |
| trajectory_steps | 领域状态或执行证据，必须可审计 |
| dom_snapshots | 领域状态或执行证据，必须可审计 |
| action_approvals | 领域状态或执行证据，必须可审计 |
| site_policies | 领域状态或执行证据，必须可审计 |
...
202 Accepted
{
  ...
  "status": "queued",
  "trace_id": "tr_abc",
  "stream_url": "/v1/browser-tasks/tsk_123/events"
}
```

| 端点 | 语义 |
| --- | --- |
| POST /v1/browser-tasks | 创建任务；必须支持 Idempotency-Key |
| GET /v1/browser-tasks/{id} | 查询状态、预算、当前阻塞原因 |
| GET /v1/browser-tasks/{id}/events | SSE 订阅 step、usage、approval、done |
| POST /v1/browser-tasks/{id}/cancel | 取消未完成任务，已执行副作用不回滚 |
| POST /v1/browser-tasks/{id}/approvals | 审批或拒绝高风险动作 |
...
services:
  api:
    ...
    environment:
      OTEL_SERVICE_NAME: project-05-ai-browser-agent-api
    ports: ["8080:8080"]
  worker:
...
- 控制：浏览器沙箱与网络 egress allowlist。
- 控制：页面内容永远作为 untrusted data。
- 控制：动作策略引擎。
- 控制：凭据隔离 vault。
- 控制：HITL 审批。
...
- 失败案例：DOM 动态变化导致 locator 失效。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：A/B test 改版。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：登录态过期。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：弹窗/验证码。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：模型被页面文案注入。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
...
- AI Browser Agent 的生产核心是受治理的长任务状态机，不是一次聊天补全。
