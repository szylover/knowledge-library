...
> AI CRM 不是给销售写几句漂亮话，而是把非结构化客户信号转成可信、可回滚、可审批的业务状态变更。

## Business Goal

本项目服务的不是 demo 场景，而是企业里的销售与客户运营工作流。目标用户是销售、Customer Success、RevOps、增长团队；他们的痛点不是不会点按钮或不会写总结，而是流程分散、证据不足、成本不可控、权限边界模糊。

业务目标可以压缩成一句话：从邮件、会议、工单、聊天记录中抽取客户状态，给出下一步行动建议并安全写回 CRM，并且把每一次模型判断、工具调用、人工批准都变成可追踪的系统事件。
...
关键风险：PII 与商业敏感信息泄漏，或错误 tool call 覆盖真实商机状态。这决定了架构必须默认不信任外部内容，默认限制动作空间，默认把高风险行为交给 Human-in-the-loop。

- 参考：Part 1 Chapter 01 API 设计：幂等与版本化。
- 参考：Part 2 Chapter 01 LLM 基础：无状态模型与上下文预算。
- 参考：Part 2 Structured Output/评测章节：抽取质量回归。
...
- 作为业务用户，我可以提交一个自然语言目标，系统异步执行销售与客户运营任务，并实时看到进度。
...
ai-project/
  ...
  deploy/
  app/api/insights.py
  app/ingest/connectors.py
  app/extract/schema.py
  app/agent/graph.py
  app/scoring/lead_score.py
  app/crm/salesforce.py
  app/security/pii.py
  app/db/models.py
...
```text
SYSTEM:
You are the planning component for AI CRM.
...
工作流核心是 `ingest → normalize → extract → retrieve history → score → recommend → approve → sync`。不要把它实现成 while True + prompt；生产系统需要显式节点、可持久化状态、可恢复错误和明确终止条件。

```mermaid
flowchart TD
    N0[Ingest Interaction]
    N0[Ingest Interaction] --> N1[PII Classify]
    N1[PII Classify] --> N2[Extract Entities]
    N2[Extract Entities] --> N3[Retrieve Account Memory]
    N3[Retrieve Account Memory] --> N4[Compute Lead Score]
    N4[Compute Lead Score] --> N5[Recommend Next Best Action]
    N5[Recommend Next Best Action] --> N6[Approval Gate]
    N6[Approval Gate] --> N7[CRM Writeback]
    N7[CRM Writeback] --> N8[Audit]
...
class ContactSignal(BaseModel):
    ...
    name: str | None = None
    role: str | None = None
    buying_intent: Literal["none", "low", "medium", "high"]
    objections: list[str] = []
    next_step: str | None = None
    evidence_message_id: str

class CrmMutation(BaseModel):
    account_id: str
    operation: Literal["create_task", "update_stage", "add_note"]
    payload: dict
    requires_approval: bool = True
    reason: str = Field(min_length=10)

async def prepare_writeback(signal: ContactSignal, account_id: str) -> CrmMutation:
    if signal.buying_intent == "high" and signal.next_step:
        return CrmMutation(
            account_id=account_id,
            operation="create_task",
            payload={"title": signal.next_step, "source": signal.evidence_message_id},
            reason="customer expressed high buying intent with explicit next step",
        )
    return CrmMutation(
        account_id=account_id,
        operation="add_note",
        payload={"summary": signal.model_dump(exclude_none=True)},
        reason="record extracted customer signal without changing forecast",
    )
```
...
本项目的 RAG 不是“相似度搜索补充上下文”，而是为 agent 提供可引用、可过滤、可过期的业务记忆：客户历史、合同、会议纪要、工单、产品使用事件、行业 playbook、销售方法论。
...
| 实体 | 用途 |
| --- | --- |
| accounts | 领域状态或执行证据，必须可审计 |
| contacts | 领域状态或执行证据，必须可审计 |
| interactions | 领域状态或执行证据，必须可审计 |
| extractions | 领域状态或执行证据，必须可审计 |
| lead_scores | 领域状态或执行证据，必须可审计 |
| crm_sync_jobs | 领域状态或执行证据，必须可审计 |
...
202 Accepted
{
  ...
  "status": "queued",
  "trace_id": "tr_abc",
  "stream_url": "/v1/crm-insights/tsk_123/events"
}
```

| 端点 | 语义 |
| --- | --- |
| POST /v1/crm-insights | 创建任务；必须支持 Idempotency-Key |
| GET /v1/crm-insights/{id} | 查询状态、预算、当前阻塞原因 |
| GET /v1/crm-insights/{id}/events | SSE 订阅 step、usage、approval、done |
| POST /v1/crm-insights/{id}/cancel | 取消未完成任务，已执行副作用不回滚 |
| POST /v1/crm-insights/{id}/approvals | 审批或拒绝高风险动作 |
...
services:
  api:
    ...
    environment:
      OTEL_SERVICE_NAME: project-07-ai-crm-api
    ports: ["8080:8080"]
  worker:
...
- 成本驱动：海量历史交互 embedding。
- 成本驱动：结构化抽取重试。
- 成本驱动：多租户 RAG 查询。
- 成本驱动：CRM API rate-limit 回退。
...
- 失败案例：联系人消歧错误。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：抽取 schema 漂移。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：历史记录过长。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：CRM API 幂等失败。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：销售代表反馈不闭环。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
...
- AI CRM 的生产核心是受治理的长任务状态机，不是一次聊天补全。
