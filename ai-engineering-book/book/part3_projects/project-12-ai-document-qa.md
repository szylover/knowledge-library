# Project 12 — AI Document QA

> Document QA 的核心不是把 PDF 丢进向量库，而是 per-user 隔离、增量解析、多模态表格/图形理解、引用级答案和 faithfulness 评估；它和组织级 KB RAG 是两个产品。

---

## Business Goal

面向用户上传文档的临时或长期问答系统，支持 PDF/Office/图片，提供 citation-grounded answer、文档级 ACL、增量 ingestion 和会话级隔离。

本章默认读者已经理解后端系统、分布式事务、缓存、队列和基本 LLM 概念；重点放在生产约束、失败域、质量闭环和成本治理。

交叉参考：Part1 Ch01 资源化 API、Part1 Ch05 存储设计、Part2 Ch07 RAG、Part2 Ch08 Multimodal、Part2 Ch15 Evaluation、Part2 Ch19 Safety。

| 维度 | 生产目标 | 不做什么 |
|---|---|---|
| 用户价值 | answer faithfulness、citation coverage、ingestion latency、retrieval recall、per-document cost。 | 不把 demo success 当作上线指标 |
| 平台价值 | 可审计、可回放、可限额 | 不把 prompt 当作唯一业务逻辑 |
| 工程价值 | 故障可恢复，质量可度量 | 不依赖人工看日志猜测问题 |
| 商业价值 | 单次任务毛利可计算 | 不让模型、图片或 token 成本黑盒增长 |

关键风险：跨用户文档泄露、表格解析错误导致错误结论、引用不支持答案、上传恶意文件、增量索引不一致。
...
### User stories
- 作为终端用户，我可以提交 document ingestion and citation-grounded QA 请求并实时看到阶段状态，而不是等待一个黑盒 HTTP 超时。
...
flowchart LR
    ...
    R --> W[docqa-ingestion-worker\nLangGraph Workers]
    W --> MG[Model Gateway\nOpenAI · Anthropic · Fallback]
    W --> V[(Qdrant\ndocqa_{tenant}_{user})]
    W --> OBJ[Postgres metadata + object storage original files + Qdrant vectors]
    W --> POL[Policy Engine\nGuardrails · HITL · DLP]
...
- 答案必须绑定 page/span/table cell citation；无法被引用支持时返回 insufficient evidence，而不是生成看似合理的总结。
...
| LLM | multimodal parser + text embedding + reranker + answer LLM with citation verifier | 质量、延迟和成本按任务路由 |
...
```text
SYSTEM:
You are the orchestration component for Project 12 AI Document QA.
...
TOOLS: pdf_parse, office_convert, ocr_extract, table_detect, retrieve_chunks, answer_with_citations
```
...
用户上传文档、页级 OCR、表格结构、图片 caption、版本化 chunk；区别于 P02 的 org-wide KB，这里默认 per-user/per-session scope。
...
services:
  ...
  worker:
    ...
    environment:
      QUEUE_NAME: docqa-ingestion-worker
      MAX_CONCURRENCY: "8"
  redis:
    image: redis:7
  postgres:
...
安全边界包括：doc ACL、malware scan、prompt injection isolation、citation required、retention control。AI 系统的特殊点是外部内容可能试图改变模型行为，因此 data 和 instruction 必须强隔离。
...
- AI Document QA 的工程重点是边界治理，不是把 LLM 接到 UI。
