# Project 08 — AI Email Assistant

> 邮件 assistant 的底线不是“写得像人”，而是永远不把错误收件人、错误附件、错误承诺自动发出去。

## Business Goal

本项目服务的不是 demo 场景，而是企业里的企业邮件与个人生产力工作流。目标用户是高频邮件用户、销售、管理者、支持团队；他们的痛点不是不会点按钮或不会写总结，而是流程分散、证据不足、成本不可控、权限边界模糊。

业务目标可以压缩成一句话：自动 triage、线程摘要、草稿生成、日程协调，并在用户批准后执行发送/日历操作，并且把每一次模型判断、工具调用、人工批准都变成可追踪的系统事件。
...
关键风险：邮件正文是天然不可信输入，prompt injection 可诱导 assistant 泄漏历史邮件或跳过审批。这决定了架构必须默认不信任外部内容，默认限制动作空间，默认把高风险行为交给 Human-in-the-loop。

- 参考：Part 1 Chapter 01 API 设计：流式草稿与异步任务。
- 参考：Part 2 Chapter 01 LLM 基础：上下文预算与无状态。
- 参考：Part 2 安全章节：不可信内容隔离。
...
- 作为业务用户，我可以提交一个自然语言目标，系统异步执行企业邮件与个人生产力任务，并实时看到进度。
...
ai-project/
  ...
  app/agent/graph.py
  app/draft/voice.py
  app/policy/send_guard.py
  app/calendar/tools.py
  app/db/models.py
  app/workers/mailbox_worker.py
  tests/eval/test_send_safety.py
```
...
```text
SYSTEM:
You are the planning component for AI Email Assistant.
...
工作流核心是 `sync → classify → summarize thread → retrieve user memory → draft/plan action → verify → approve → send`。不要把它实现成 while True + prompt；生产系统需要显式节点、可持久化状态、可恢复错误和明确终止条件。

```mermaid
flowchart TD
    N0[Mailbox Event]
    N0[Mailbox Event] --> N1[Classify Priority]
    N1[Classify Priority] --> N2[Summarize Thread]
    N2[Summarize Thread] --> N3[Retrieve Voice/Context]
    N3[Retrieve Voice/Context] --> N4[Draft Reply]
    N4[Draft Reply] --> N5[Policy/Recipient Check]
    N5[Policy/Recipient Check] --> N6[Need Approval]
    N6[Need Approval] --> N7[Send/Calendar Tool]
    N7[Send/Calendar Tool] --> N8[Audit]
    N8 --> END[Terminal State]
...
class DraftPolicyResult(BaseModel):
    allowed: bool
...
SENSITIVE_TERMS = {"pricing", "contract", "password", "token", "bank"}

def verify_before_send(draft: EmailDraft, external_domains: set[str]) -> DraftPolicyResult:
    body = draft.body_markdown.lower()
    recipients = draft.to + draft.cc
    external = [r for r in recipients if r.split("@")[-1] in external_domains]
    reasons: list[str] = []
    if external and any(term in body for term in SENSITIVE_TERMS):
        reasons.append("sensitive content to external domain")
    if len(recipients) > 10:
        reasons.append("large recipient set")
    return DraftPolicyResult(allowed=not reasons, risk="high" if reasons else "low", reasons=reasons)
```
...
本项目的 RAG 不是“相似度搜索补充上下文”，而是为 agent 提供可引用、可过滤、可过期的业务记忆：用户写作风格、联系人关系、历史线程、公司政策、附件摘要、日历上下文。
...
| 实体 | 用途 |
| --- | --- |
| mailboxes | 领域状态或执行证据，必须可审计 |
| email_threads | 领域状态或执行证据，必须可审计 |
| messages | 领域状态或执行证据，必须可审计 |
| drafts | 领域状态或执行证据，必须可审计 |
| user_voice_profiles | 领域状态或执行证据，必须可审计 |
| send_approvals | 领域状态或执行证据，必须可审计 |
...
202 Accepted
{
  ...
  "status": "queued",
  "trace_id": "tr_abc",
  "stream_url": "/v1/email-assistant/tsk_123/events"
}
```

| 端点 | 语义 |
| --- | --- |
| POST /v1/email-assistant | 创建任务；必须支持 Idempotency-Key |
| GET /v1/email-assistant/{id} | 查询状态、预算、当前阻塞原因 |
| GET /v1/email-assistant/{id}/events | SSE 订阅 step、usage、approval、done |
| POST /v1/email-assistant/{id}/cancel | 取消未完成任务，已执行副作用不回滚 |
| POST /v1/email-assistant/{id}/approvals | 审批或拒绝高风险动作 |
...
services:
  api:
    ...
    environment:
      OTEL_SERVICE_NAME: project-08-ai-email-assistant-api
    ports: ["8080:8080"]
  worker:
...
- 控制：OAuth scope 最小化。
- 控制：邮件内容 untrusted 标记。
- 控制：recipient/domain guardrail。
- 控制：附件扫描。
- 控制：发送前 HITL。
...
- 失败案例：线程引用错位。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：回复全部误触。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：附件遗漏。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：时区错误。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
- 失败案例：邮件内 prompt injection。对应措施是把它显式写进 eval、policy 或 recovery workflow，而不是靠 prompt 祈祷。
...
- AI Email Assistant 的生产核心是受治理的长任务状态机，不是一次聊天补全。
